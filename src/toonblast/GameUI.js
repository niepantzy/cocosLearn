var GameUI = cc.Layer.extend({
    ...
    _initInfoPanel : function () {
        ...
        var targetLabel = new cc.LabelTTF("Target","arial",36);
        targetLabel.x = 450;
        targetLabel.y = levelLabel.y;
        targetLabel.setColor(cc.color(0,0,0));
        this.addChild(targetLabel);

        var targetText = new cc.LabelTTF("1","arial",36);
        targetText.x = 450;
        targetText.y = levelText.y;
        targetText.setColor(cc.color(0,0,0));
    ...
    showSuccessAll : function () {
        var bg = new cc.LayerColor(cc.color(255,255,255),500,500);
        this.addChild(bg,1);
        var size = cc.director.getWinSize();
        bg.x = (size.width - bg.width)/2;
        bg.y = (size.height = bg.height)/2;
        var  stepText = new cc.LabelTTF("恭喜您已经通关！","arial",50);
        stepText.setColor(cc.color(0,0,0));
        stepText.x = 250;
        stepText.y = 250;
        bg.addChild(stepText);
